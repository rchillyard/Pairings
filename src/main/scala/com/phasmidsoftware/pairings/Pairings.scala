...
object Player extends TableParserHelper[Player]() {
    ...
    def convertTable(pt: Table[Player]): Table[Partnership] = {
        val pst: Table[Iterable[Player]] = pt.processRows(xs => (xs grouped 2).toSeq)
        val zst: Table[Partnership] = pst.processRows(z => z.flatMap(Partnership.create))
        zst.replaceHeader(Some(Header.create("playerA", "playerB")))
    }
}
